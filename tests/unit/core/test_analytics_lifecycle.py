...
def test_lifespan_publishes_analytics_engine() -> None:
    pools = StubDatabasePools()
    provider = DeterministicMockLLMProvider(
        model_name="analytics-lifespan-model",
    )
    ...
    def analytics_engine_factory() -> DeterministicAnalyticsEngine:
    ...
    application = create_app(
        settings=Settings(
            llm_provider="mock",
            llm_model="analytics-lifespan-model",
        ),
        pool_factory=pool_factory,
        llm_provider_factory=provider_factory,
        analytics_engine_factory=analytics_engine_factory,
    )

    with TestClient(application):
        assert application.state.analytics_engine is engine
...
def test_analytics_engine_factory_failure_closes_provider() -> None:
    pools = StubDatabasePools()
    provider = DeterministicMockLLMProvider(
        model_name="analytics-lifespan-model",
    )
    ...
    def failing_analytics_engine_factory() -> DeterministicAnalyticsEngine:
        nonlocal engine_factory_calls
        engine_factory_calls += 1
        raise RuntimeError("Analytics engine construction failed")

    application = create_app(
        pool_factory=pool_factory,
        llm_provider_factory=provider_factory,
        analytics_engine_factory=failing_analytics_engine_factory,
    )

    with (
        pytest.raises(
            RuntimeError,
            match="Analytics engine construction failed",
        ),
