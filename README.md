...
- `POST /api/v1/presentations/generate` returns the consolidated presentation JSON;
- `POST /api/v1/presentations/export` returns an in-memory PowerPoint download;
...
## In-memory PowerPoint export

`POST /api/v1/presentations/export` accepts the same strict, question-only
`PresentationRequest`. Filename, path, format, SQL, rows, and artifact payloads
are not accepted. The endpoint runs the controlled presentation pipeline, then
passes its internal result to the artifact service. The pipeline can access the
configured database and LLM provider; only the subsequent PPTX generation is
offline, with no database, network, LLM, filesystem writes, or temporary files.

The generator uses pinned `python-pptx==1.0.2` and in-memory buffers. It
revalidates nested result contracts and source consistency before projecting
validated KPI values, table specifications, vector bar and line shapes, grounded
claim text, evidence identifiers, source versions, and source row count. It does
not recalculate analytical metrics or embed chart workbooks. The output is
limited to 20 slides and 5,242,880 bytes (5 MiB).

Successful responses use the fixed PowerPoint MIME type, an attachment filename
matching `analytical-presentation-[0-9a-f]{24}.pptx`, a content length,
`Cache-Control: no-store`, and `X-Content-Type-Options: nosniff`.
The artifact identifier is derived from semantic content, not a checksum of the
ZIP bytes. Repeatability means stable identity and equivalent semantic content;
identical bytes across environments are not promised.

Exported content excludes raw or validated SQL, raw query rows, grounding
context, prompts, and raw provider responses. This projection does not change
`/presentations/generate`: its JSON still contains validated SQL and projected
query rows, which the existing frontend can display. Qualitative narrative
remains provider-authored and is not independently truth-scored.

The OOXML validator checks allowed package members, XML, content types, and
internal relationship targets. It rejects unsupported parts, duplicate or unsafe
member names, external relationships, dangling targets, macros, OLE objects, and
embedded packages. Two fixed template auxiliaries are hash-checked exceptions:
`docProps/thumbnail.jpeg` and
`ppt/printerSettings/printerSettings1.bin`. The thumbnail is not a rendered
slide image. No public PPTX upload endpoint is provided.

Local validation reopened the generated bytes with python-pptx and exercised
65 isolated adversarial cases. It did not validate desktop PowerPoint or
LibreOffice rendering. See `docs/operations.md` for download headers, failure
handling, and remaining operational limits.
...
- 725 automated tests passed;
- branch-aware backend coverage is 88.28%;
- Ruff lint and format checks pass;
- mypy strict mode passes;
- earlier dependency auditing found no known dependency vulnerabilities;
- earlier secret scanning found no local secret values in versionable source files.

The post-exporter 1.11-E local gate passed on Python 3.13.5. Its full suite
include the 30 artifact tests and the 118 presentation-focused tests; these
counts are not additive. Coverage combines covered statements and branches,
with the existing 80% threshold unchanged. Dependency auditing and secret
scanning were not rerun in this gate. The 65-case isolated adversarial matrix
passed separately after 1.11-D4-R1; remote CI and container validation for these
uncommitted exporter changes remain pending.
...
The validated local baseline contains **725 passing tests**. The CI matrix
